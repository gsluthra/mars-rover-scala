...
case class MarsRover(plateau: Plateau, currentPoint: Coordinates, face: Direction) {
  ...
  def run(commandString: String) = {
    val commands = StringCommandParser(Option(commandString)).commands
  ...
  def turnLeft() = MarsRover(plateau, currentPoint, face.left)

  def turnRight() = MarsRover(plateau, currentPoint, face.right)

  def currentLocation: String = toString

  override def toString: String = s"(${currentPoint.x}, ${currentPoint.y}, ${face})"
